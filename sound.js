export function soundMix(state,time,{paused=false,hidden=false,narrating=false}={}){
  ...
  const moving=state.truckMoving||state.dumpPhase==='returning'||state.buses.some(b=>b.phase==='returning')||(state.riding&&['moving','train-moving'].includes(state.mode));
  const emergency=state.riding&&['police','ambulance'].includes(state.vehicle);
  return {
    engineHz:(state.drivingTrain?50:state.drivingBus?58:state.drivingDump?72:82)+Math.sin(time*23)*1.5,
